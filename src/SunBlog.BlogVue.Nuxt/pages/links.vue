<template>
  ...
  <v-card class="blog-container">
    ...
    <blockquote>
      <div>名称：{{ site?.result?.site?.siteName }}</div>
      <div>简介：{{ site?.result?.site?.description }}</div>
      <div>头像：{{ site?.result?.site?.logoUrl ?? "/default.jpg" }}</div>
    </blockquote>
...
useSeoMeta({
  title: "友情链接-" + site.value?.result?.site?.siteName,
