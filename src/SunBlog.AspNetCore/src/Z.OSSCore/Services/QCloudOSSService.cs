...
using Z.OSSCore.Models.Exceptions;
using Z.OSSCore.Models.Policy;
using Z.OSSCore.Providers;

namespace Z.OSSCore.Services
{
    public class QCloudOSSService<T>  : BaseOSSService, IQCloudOSSService<T> 
    {
        ...
        public async Task<ObjectOutPut> GetObjectAsync(GetObjectInput input)
        {
            ...
            GetObjectBytesRequest request = new GetObjectBytesRequest(input.BucketName, input.BucketName);
            //执行请求
            GetObjectBytesResult result = _client.GetObject(request);
            byte[] content = result.content;
            ...
            string contentType = result.responseHeaders["Content-Type"].FirstOrDefault() ?? "application/octet-stream";
        ...
        public async Task<bool> UploadObjectAsync(UploadObjectInput input)
        {
            ...
            bool found = await BucketExistsAsync(input.BucketName);
            //如果桶不存在则创建桶
            if (!found)
                await CreateBucketAsync(input.BucketName);
            input.ObjectName = FormatObjectName(input.ObjectName);
            input.BucketName = ConvertBucketName(input.BucketName);
            PutObjectRequest request = new PutObjectRequest(input.BucketName, input.ObjectName, input.Stream);
        ...
        public Task<bool> RemoveObjectAsync(OperateObjectInput input)
        {
            ...
            input.ObjectName = FormatObjectName(input.ObjectName);
            input.BucketName = ConvertBucketName(input.BucketName);
            DeleteObjectRequest request = new DeleteObjectRequest(input.BucketName, input.ObjectName);
        ...
        public Task<string> PresignedGetObjectAsync(string bucketName, string objectName, int expiresInt)
        {
            return PresignedObjectAsync(bucketName
                ...
                , async (bucketName, objectName, expiresInt) =>
                {
                    ...
                    //生成URL
                    AccessMode accessMode = await this.GetObjectAclAsync(new OperateObjectInput
                    {
        ...
        public async Task<AccessMode> GetObjectAclAsync(OperateObjectInput input)
        {
            ...
            bool isPublicRead = false;
            bool isPublicWrite = false;
            GetObjectACLResult result = _client.GetObjectACL(new GetObjectACLRequest(ConvertBucketName(input.BucketName), input.ObjectName));
            AccessControlPolicy acl = result.accessControlPolicy;
        ...
        public Task<AccessMode> RemoveObjectAclAsync(OperateObjectInput input)
        {
            ...
            input.ObjectName = FormatObjectName(input.ObjectName);
            PutObjectACLRequest request = new PutObjectACLRequest(ConvertBucketName(input.BucketName), input.ObjectName);
            //设置私有读写权限 
            request.SetCosACL("default");
            PutObjectACLResult result = _client.PutObjectACL(request);
            if (result.IsSuccessful())
            {
                return GetObjectAclAsync(input);
            }
