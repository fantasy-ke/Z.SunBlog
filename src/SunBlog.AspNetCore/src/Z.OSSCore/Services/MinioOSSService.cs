...
namespace Z.OSSCore.Services
{
    public class MinioOSSService<T> : BaseOSSService, IMinioOSSService<T>
    {
            ...
            : base(cache, options)
        {
            ...
            _client = (MinioClient)client.Build();
        }
        ...
        public async Task<PolicyInfo> GetPolicyAsync(string bucketName)
        {
            try
            {
                ...
                return policyJson.ToObject<PolicyInfo>();
            }
        ...
        public async  Task<ObjectOutPut> GetObjectAsync(GetObjectInput input)
        {
            ...
            var statObj = await _client.GetObjectAsync(getObjectArgs,input.CancellationToken);
        ...
        public async Task<bool> RemoveObjectAsync(string bucketName, List<string> objectNames)
        {
            ...
            IObservable<DeleteError> observable = await _client.RemoveObjectsAsync(args);
        ...
        public Task<string> PresignedGetObjectAsync(string bucketName, string objectName, int expiresInt)
        {
            return PresignedObjectAsync(bucketName
                ...
                , async (bucketName, objectName, expiresInt) =>
                {
                    objectName = FormatObjectName(objectName);
                    //生成URL
                    AccessMode accessMode = await GetObjectAclAsync(new OperateObjectInput()
                    {
        ...
        public async Task<AccessMode> GetObjectAclAsync(OperateObjectInput input)
        {
            ...
            input.ObjectName = FormatObjectName(input.ObjectName);
            if (!input.ObjectName.StartsWith(input.BucketName))
            {
                input.ObjectName = $"{input.BucketName}/{input.ObjectName}";
            }
            //获取存储桶默认权限
