...
namespace Z.OSSCore.Interface.Base
{
    public abstract class BaseOSSService
    {
        ...
        public virtual Task RemovePresignedUrlCache(OperateObjectInput input)
        {
            ...
            {
                string key = BuildPresignedObjectCacheKey(input.BucketName, input.ObjectName, PresignedObjectType.Get);
                _cache.Remove(key);
            ...
            , Func<string, string, int, Task<string>> PresignedFunc)
        {
            try
            {
                ...
                if (Options.IsEnableCache && expiresInt > minExpiresInt)
                {
                    ...
                    //Unix时间
                    long nowTime = TimeExtensions.Timestamp();
                    //缓存中存在，且有效时间不低于10分钟
                    if (cache != null
                        && cache.Type == type
                        && cache.CreateTime > 0
        ...
        {
            return "OSS:" + MD5Encryption.Encrypt($"{GetType().FullName}_{bucketName}_{objectName}_{type.ToString().ToUpper()}");
        }
    }
}
